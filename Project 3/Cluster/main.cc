...
int main(int argc, char** argv) {
    ...
    if (reduced_input_file != "") {
        points = readfile(reduced_input_file, 10000);
    }
    else { 
        points = readfile(input_file, 10000);
    }
    ...
    std::cout << "clustering_time: " << duration.count()/1000.0 << std::endl;
    ...
    std:: cout << "Silhouette: [";
    for (unsigned int i = 0; i < silhouette.size(); i++){
        std::cout << silhouette[i] << " ";
    }
    std::cout << "]" << std::endl;

    double objective_function = kmeans.objective_function();

    std::cout << "Objective Function: " << objective_function << std::endl;

    output << "Objective Function: " << objective_function << std::endl;   
